...
def serialize_python_rng_state() -> dict[str, torch.Tensor]:
    ...
    return {
        "py_rng_version": torch.from_numpy(np.asarray([py_state[0]], dtype=np.int64)),
        "py_rng_state": torch.from_numpy(np.asarray(py_state[1], dtype=np.int64)),
    }
...
def serialize_numpy_rng_state() -> dict[str, torch.Tensor]:
    ...
    return {
        ...
        "np_rng_state_index": torch.from_numpy(np.asarray([np_state[2]], dtype=np.int64)),
        "np_rng_has_gauss": torch.from_numpy(np.asarray([np_state[3]], dtype=np.int64)),
        "np_rng_cached_gaussian": torch.from_numpy(np.asarray([np_state[4]], dtype=np.float32)),
    }
